...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ name, identifier, service, jsonId, user, filename, fullFile }) => {
    ...
    const handleProgressClick = (e: Event, newValue: number | number[]) => {
        const audio = audioRef.current;
        const clickPositionInPercentage = Array.isArray(newValue) ? newValue[0] : newValue;
    ...
    return (
        <PlayerBox>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', pl: 1, pr: 1 }}>
                {fullFile && (
                      <FileElement
                      fileInfo={fullFile}
